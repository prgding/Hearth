...
enum PnLFormatter {
    ...
    /// Plain grouped value (no +/- prefix) — for market values, not deltas.
    static let value: NumberFormatter = {
        let f = NumberFormatter()
        f.numberStyle = .decimal
        f.maximumFractionDigits = 2
        f.minimumFractionDigits = 2
    ...
    static func valueString(_ v: Double) -> String {
        value.string(from: NSNumber(value: v)) ?? "0.00"
    }
